...
setInterval(autoDisableFilters, 3000);

// 📢 IPC HANDLERS
ipcRenderer.on('veriy-mode-sync', (event, status) => {
    // Left for dashboard compatibility
});

ipcRenderer.on('mass-message-inject', (event, text) => {
...
ipcRenderer.on('setup-session', (event, token) => { 
    if (token && token.length > 50) {
        ...
        if (cw.localStorage.getItem('token') !== t) {
            cw.localStorage.setItem('token', t);
            cw.localStorage.setItem('multi-account-tokens', JSON.stringify({ [token]: token }));
